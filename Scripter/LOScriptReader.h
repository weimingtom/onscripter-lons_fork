...
public:
	...
	void ReadyToRun(LOString *lname, int callby = LOScriptPoint::CALL_BY_NORMAL);
	int ReadyToBack();
	...
	static LOStack<LOScriptReader> readerList;  //脚本解析分组
	static bool scripterhasExit;
private:
	static LOStack<LOScriptReader> workingList;    //处于活动状态的脚本解析分组
	...
	//LOEventLink eventLink;
	

	LOScriptReader* GetPollReader();
	LOScriptPoint* GetScriptPoint(LOString lname);
	int ContinueRun();
	int ContinueEvent();
//	void RunInserCommand(LOEvent *e);
	int MainStartParse();  //
