...
public:
	...
	enum {
		...
		IDEX_NSSYS_BTN = 6,    //btn定义的按钮
		IDEX_NSSYS_CUR = 7,    //Cursor在的层
		IDEX_NSSYS_RMENU = 20,
	};
	...
	//LOLayerInfo *origInfo;      //原始的当前显示数据
	...
	//获取一个没有被占用的子图层对象id，由调用者做有效性检查
	int GetUnusedChild(int from,int step);
...
private:
	static std::unordered_map <std::string, LOtexture*> texMap;        //材质缓存
