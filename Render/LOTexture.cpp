...
LOtextureBase::~LOtextureBase() {
	...
	for (int ii = 0; ii < texlist.size(); ii++) {
		if (ii % 2 == 0) delete ((SDL_Rect*)texlist[ii]);
			...
			SDL_DestroyTexture((SDL_Texture*)texlist[ii]);
		}
	}
	texlist.clear();
}
...
void LOtextureBase::Init() {
	SDL_Texture* tx = SDL_CreateTextureFromSurface(render, su->GetSurface());
...
SDL_Texture* LOtextureBase::GetTexture(SDL_Rect *re) {
	...
	if (!texture) {
		...
		texture = SDL_CreateTextureFromSurface(render, tsu->GetSurface());
		delete tsu;
...
//只能运行在主线程
LOtexture* LOtexture::addNewEditTexture(LOString &fname, int w, int h, Uint32 format, SDL_TextureAccess access) {
	LOString s = fname.toLower();
	SDL_Texture *tx = SDL_CreateTexture(LOtextureBase::render, format, access, w, h);
	LOtextureBase *base = new LOtextureBase(tx);
	LOtexture::baseMap[s] = base;
	return new LOtexture(base);
}
...
LOtexture::LOtexture(int w, int h, Uint32 format, SDL_TextureAccess access) {
	NewTexture();
	SDL_Texture *t = SDL_CreateTexture(LOtextureBase::render, format, access, w, h);
...
bool LOtexture::activeActionTxtTexture() {
	...
	int w = su->W();
	int h = su->H();
	SDL_Texture *tx = SDL_CreateTexture(LOtextureBase::render, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STREAMING, w, h);
	if (!tx) return false;
